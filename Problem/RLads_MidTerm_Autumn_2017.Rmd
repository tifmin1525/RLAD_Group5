...
plot(mtcars$mpg, mtcars$hp, 
     ...
     main = "Scatter plot of Miles/gallon and Hourse Power", 
     col ="blue", pch=16)
```
...
Orange %>%
    group_by(Tree) %>%
    summarise(cir.diff=max(circumference)-min(circumference)) %>%
    arrange(desc(cir.diff))
```
...
textStr <- "Alice, Taiwan, TEL:02-33663366, EMail: anonymous@ntu.edu.tw"
# split the string
fieldValues <- strsplit(textStr, ",")[[1]]

# extract the substring
TEL <- substr(fieldValues[3], 6, nchar(fieldValues[3]))
...
# copy-paste the code above and fill out the blank
data.id.vec <- c("b52123045", "b31a01221", "b12z03013", "b24054365", "breeze002")
grep("b\\d\\d[0-9,a]", data.id.vec, value=T)
```
...
# find the pattern with `gregexpr` and extract the matched string with a function
stock.symbols <- regmatches(fin.news, gregexpr("\\(\\w+,", fin.news))[[1]]
sapply(stock.symbols, function(x) substr(x, 2, nchar(x)-1), USE.NAMES=FALSE)
```
...
hist(mtcars$hp, main = "Histogram of horsepower in mtcars", xlab = "Horsepower")
```
...
df.hp <- mtcars %>%
  select(cyl, hp) %>%
  group_by(cyl) %>%
  summarise(n = length(cyl), hp_avg = mean(hp), hp_std = sd(hp))
# please show the results with the line below
print(df.hp)
```
...
```{r ans.2.3}
# please put your answer here
df <- readr::read_file("/Volumes/BaseQi/Problem/access_log_nasa_95_1k.txt")
df <- strsplit(df, "\n")
df <- unlist(df)
m <- gregexpr("/images/", df)
dfout <- regmatches(df, m)
n.images <- length(unlist(dfout))
m <- gregexpr("\\.jp ", df)
dfout <- regmatches(df, m)
n.jp <- length(unlist(dfout))
# please show the results with the line below
item2.ans <- paste(
...
# please put your answer here
library(tibble)
df <- readr::read_file("/Volumes/BaseQi/Problem/lyrics.txt")
seq <- worker()
df <- seq[df]
df.lyrics <- freq(df)
df.lyrics <- df.lyrics %>%
  filter(freq > 1) %>%
  arrange(desc(freq))
colnames(df.lyrics) <- c("word", "freq")
df.lyrics <- as.tibble(df.lyrics)
# please show the results with the line below
print(df.lyrics)
```
