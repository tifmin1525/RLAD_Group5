...
```{r}
movies_data <- read_csv(file.choose())
movies_data <- na.omit(movies_data)
a <- movies_data %>%
    filter(budget < 253632000) %>%
    select(genres, budget, imdb_score, movie_title) %>%
    group_by(movie_title, genres) 
a
plot(a$budget, a$imdb_score)
